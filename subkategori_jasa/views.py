...
def get_pekerja_by_id(pekerja_id):
    pekerja = next(
        (pekerja for pekerja in DUMMY_PEKERJA if pekerja["id"] == pekerja_id), None
    )
    if not pekerja:
        return {"nama": "Tidak Diketahui"}
    return pekerja
...
def get_status_pesanan_by_id_tr_pemesanan(pemesanan_id):
    ...
    if status:
        return next(
            (st for st in DUMMY_STATUS_PESANAN if st["id"] == status["id_status"]), None
        )
    return None
...
def subkategori_jasa_pelanggan(request, subkategori_id):
    ...
    if not subkategori:
        return render(
            request,
            "subkategori_jasa/not_found.html",
    ...
    testimoni = [
        ...
        if t["id_tr_pemesanan"] == p["id"]
        and p["id_kategori_jasa"] == subkategori["kategori_jasa_id"]
    ]
...
def subkategori_jasa_pekerja(request, subkategori_id):
    ...
    pekerja_list = [
        ...
        if any(
            pk["pekerja_id"] == pekerja["id"]
            and pk["kategori_jasa_id"] == subkategori["kategori_jasa_id"]
    ...
    testimoni_list = [
        ...
        if tr["id"] == testimoni["id_tr_pemesanan"]
        and tr["id_kategori_jasa"] == subkategori["kategori_jasa_id"]
    ]
    ...
    if request.method == "POST":
        ...
        if user and user["role"] == "pekerja":
            ...
            if not already_joined:
                ...
                return redirect(
                    "subkategori_jasa:subkategori_jasa_pekerja",
                    subkategori_id=subkategori_id,
                )
            else:
...
def buat_pemesanan_jasa(request):
    ...
    if request.method == "POST":
        ...
        pekerja_list = get_pekerja_list_by_kategori_jasa(
            subkategori["kategori_jasa_id"]
        )
        if not pekerja_list:
            return render(
                request,
                "subkategori_jasa/error.html",
                {"message": "Tidak ada pekerja yang tersedia."},
            )
...
def profil_pekerja(request, pekerja_id):
    ...
    subkategori_id = (
        "1"  # Atur sesuai data dummy Anda jika ada relasi pekerja dengan subkategori
    )
